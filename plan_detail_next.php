...
$paymentgatParamData = [
                    ...
                    //'LocationCode' => $_SESSION['user']['ClientLocationId'],
                    'LocationCode' => 0000,
                    'Terminal' => 0000,
                ];
$payment_details_response = GetPaymentGatewayDetails($paymentgatParamData);
//print_r($payment_details_response);
if (isset($payment_details_response['ValidationDetails']['StatusCode']) &&
    $payment_details_response['ValidationDetails']['StatusCode'] == 200 &&
    !empty($payment_details_response['MasterDataList'][0])) {
    $ServerURL = $payment_details_response['MasterDataList'][0]['ServerURL'];
    $Store = $payment_details_response['MasterDataList'][0]['Store'];
    $Customer = $payment_details_response['MasterDataList'][0]['Customer'];
    $_SESSION['payment_customer'] = $Customer;
    $Terminal = $payment_details_response['MasterDataList'][0]['Terminal'];
    $UserName = $payment_details_response['MasterDataList'][0]['UserName'];
    $Password = $payment_details_response['MasterDataList'][0]['Password'];
    $_SESSION['api_UserName'] = $UserName;
    $_SESSION['api_Password'] = $Password;
}

//if(isset($_REQUEST['orderID']) && $_REQUEST['orderID'] != "" && isset($_SESSION['TransactionID']) && $_SESSION['TransactionID'] != ''){
if(isset($_SESSION['TransactionID']) && $_SESSION['TransactionID'] != ''){

    // API Endpoint
    $url = $ServerURL;
    // Your username and password
    $username = $_SESSION['api_UserName'];
    $password = $_SESSION['api_Password'];

    // Prepare the request payload
    $finali_data = [
        "Finalization" => [
            "TransactionID" => $_SESSION['TransactionID'],
            // "Customer" => $_SESSION['payment_customer']
            "Customer" => 'Demo Merchant'
        ]
    ];
    // Convert to JSON
    $jsonData = json_encode($finali_data, JSON_UNESCAPED_SLASHES);
    ...
    // Handle errors
    if (curl_errno($ch)) {
        $technical_error = " cURL Error: " . curl_error($ch);
        $_SESSION['technical_error'] = $technical_error;
    ...
    //if (isset($data['Transaction']['ResponseClassDescription']) && $data['Transaction']['ResponseClassDescription'] == "Success") {
    if (isset($data['Transaction']['ResponseCode']) && $data['Transaction']['ResponseCode'] === "0") {

        $final_data_json = json_encode($response, JSON_UNESCAPED_SLASHES);
        $finalize_response_jsonString =str_replace('"', '\"', $final_data_json);

         $package_single_details = "";
        $invoiceParam = [
        'PersonId' => $_SESSION['user']['UserId'],
        'SubscriptionId' => $_SESSION['NewSubscriptionId'],
        'TransactionId' => $_SESSION['TransactionID'],
        'BankResponse' => $finalize_response_jsonString
        ];

        //    echo "<br><br><br><br><br>before";
      //  print_r($packageParam);
        $invoicedataresp = GenerateInvoiceReceiptAgainstSubscription($invoiceParam);

       // echo "<br>after----";
       // print_r($packresponse);
        if (
            isset($invoicedataresp['ValidationDetails']['StatusCode']) &&
            $invoicedataresp['ValidationDetails']['StatusCode'] == 200 &&
            !empty($invoicedataresp['MasterDataList'][0])
        ) {

        }

        header('Location:'. ROOT_URL.'thankyou.php?group='.$_REQUEST['group'].'&package='.$_REQUEST['package'].'&mealplan='.$_REQUEST['mealplan'].'&amount='.$_SESSION['amount_val'].'&txn='.$_SESSION['TransactionID']);
        ...
        <?php
        $technical_error = $data['Transaction']['ResponseDescription'];
        $_SESSION['technical_error'] = $technical_error;
         $_SESSION['form_succ'] = 2;
       // echo "PaymentPage not found in response.";
    }
...
if(isset($_REQUEST['group']) && $_REQUEST['group'] != "" && isset($_REQUEST['package']) && $_REQUEST['package'] != ""){
    ...
    $subscriptionid_details_response = GetClientNewSubscriptionId($subgatParamData);
    if (isset($subscriptionid_details_response['ValidationDetails']['StatusCode']) &&
        $subscriptionid_details_response['ValidationDetails']['StatusCode'] == 200 &&
        !empty($subscriptionid_details_response['MasterDataList'][0])) {
        $NewSubscriptionId = $subscriptionid_details_response['MasterDataList'][0]['NewSubscriptionId'];
        $_SESSION['NewSubscriptionId'] = $NewSubscriptionId;
    }
    ...
    if (empty($card_cvv)){
        ...
        if (empty($errors))
        {
            if($ServerURL != '')
            {
                ...
                $_SESSION['amount_val'] = $subtotal;

                $_SESSION['after_payment_url'] = getCurrentUrl().'&orderID='.$_REQUEST['package'];
                $data = [
                    "Registration" => [
                        "Currency" => "AED",
                        "ReturnPath" => getCurrentUrl(),
                        "TransactionHint" => "CPT:Y;VCC:Y;",
                        "OrderID" => $_SESSION['NewSubscriptionId'],
                        "Store" => '0000',
                        "Terminal" => '0000',
                        "Channel" => "Web",
                        "Amount" => $_SESSION['amount_val'],
                        "Customer" => 'Demo Merchant',
                        "OrderName" => $package_single_details != '' ? substr($package_single_details['PackageName'], 0, 25) : 'Order name',
                        "UserName" => $UserName,
                        "Password" => $Password
                    ]
                ];
                ...
                curl_setopt_array($ch, [
                    ...
                    CURLOPT_POSTFIELDS => $jsonData,         // Attach JSON data
                    CURLOPT_HTTPHEADER => [
                        'Content-Type: application/json',
                        'Accept: application/json',
                        'Content-Length: ' . strlen($jsonData)
                    ],
                    CURLOPT_TIMEOUT => 20,                   // 20 second timeout
                    CURLOPT_SSL_VERIFYPEER => false,         // Disable SSL verify (for demo)
                    CURLOPT_SSL_VERIFYHOST => false
                ]);
                ...
                if (curl_errno($ch)) {
                    ...
                    $technical_error = 'cURL Error';
                    $_SESSION['technical_error'] = $technical_error;

                } else {
                    $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                    //echo " HTTP Status: " . $httpCode . "<br><br>";
                    //echo "<b>Response:</b><br><pre>" . htmlspecialchars($response) . "</pre>";
                }
                ...
                //print_r($data);
                //if (isset($data['Transaction']['PaymentPage']) && !empty($data['Transaction']['PaymentPage'])) {
                if (isset($data['Transaction']['ResponseCode']) && $data['Transaction']['ResponseCode'] === "0") {
               // if (isset($data['Transaction']['PaymentPage'])) {
                    $paymentPage = $data['Transaction']['PaymentPage'];
                    //echo "<br><br><br><br><br><br><br><br><br><br><br><br><br>".$paymentPage;
                    $transactionid=$data['Transaction']['TransactionID'];
                    $UniqueID=$data['Transaction']['UniqueID'];
                    $_SESSION['TransactionID'] = $data['Transaction']['TransactionID'];
                    $_SESSION['UniqueID'] = $data['Transaction']['UniqueID'];
                    
 //"ReturnPath" => getCurrentUrl().'&orderID='.$_SESSION['TransactionID'],
//https://hmimpex.com/
                    ...
                    $regdata_json = json_encode($regdata, JSON_UNESCAPED_SLASHES);                 
                    $jsonString =str_replace('"', '\"', $regdata_json);

                    $transaction_time = date('Y-m-d H:i:s');

                    $SaveBankParamData = [
                        'PersonId' => $_SESSION['user']['UserId'],
                        'PersonMobileNo' => $_SESSION['MobileNumber'],
                        'PersonEmail' => $_SESSION['user']['Email'],
                        'PackageID' => $_REQUEST['package'],
                        'Amount' => $_SESSION['amount_val'],
                        'SubscriptionId' => $_SESSION['NewSubscriptionId'],
                        'DateTimeofTransaction' => $transaction_time,
                        'TransactionId' => $_SESSION['TransactionID'],
                        'BankRequest' => $jsonString  
                        ];


                        $bank_log_response = SaveBankRequestLog($SaveBankParamData);
                        //print_r($bank_log_response);


                        if (isset($bank_log_response['StatusCode']) &&
                        $bank_log_response['StatusCode'] == 200) {
                            ...
                            $_SESSION['technical_error'] = 'save bank request log error';
                            $_SESSION['form_succ'] = 2;
                        }
                ...
                //echo "<br><br><br><br> after   json data---";
...
// Get Area
$usercityID = $clientprofileData[0]['CityId'] != '' ? $clientprofileData[0]['CityId'] : $cityid;
...
<style>
    ...
    body.loading {
        ...
        opacity: 0.5;           /* visually dim entire page */
    }
</style>
    <!-- ===========SUBHEADER=========== -->
                ...
                <?php if(isset($_SESSION['form_succ'])){ ?>
                    <div class="col-lg-12 col-lg-12 mb-lg-4" id="formMessage">
                        ...
                        <div class="message error" id="global-error-msg">
                            <?php echo $_SESSION['technical_error']; ?>
                                        ...
                                        <div class="col-lg-12 product-book mb-4">
                                            ...
                                            <div class="col-lg-6 mb-4">
                                                <div>
                                                    ...
                                                    <input type="text" name="AddressEmailId" class="form-control w-100" id="AddressEmailId" placeholder="name@example.com" value="<?= 
                                                        $AddressEmailId ?? ($clientprofileData[0]['AddressEmailId'] != '' 
                                                            ? $clientprofileData[0]['AddressEmailId'] 
                                                            : $clientprofileData[0]['EmailId']) ?>"
                                                    >
                ...
                <div class="col-lg-4">
                    ...
                    <div class="d-flex flex-column gap-3 text-white">
                        ...
                        <?php if($package_single_details['PackageMoreInfo'] != ""){ ?>
                            ...
                            <hr>
                        <?php } ?>
                        <div class="text-brand">Plan Duration: <span id="package-duration"><?php  echo $package_single_details['DurationDays']; ?></span> Days</div>
                        <hr>
                        <?php
                        $subscriptionData = [];
                        $subParamData = [
