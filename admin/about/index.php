					...
					<div class="col-md-12">		
						...
						<div class="row">
							<div class="col-md-12">
							    ...
							    <!-- Container for team member blocks -->
							        ...
							        if (isset($id)) {
							            ...
							            if ($res && $res->num_rows > 0) {
							                ...
							                foreach ($gallery_2 as $index => $item) {
							                    ...
							                    ?>
							                    <div class="col-md-6 gallery-img-box mb-3" data-filename="<?= $img ?>">
							                        <img src="<?= $gallery_url . $img ?>" class="img-thumbnail mb-1 preview-image" style="height:130px;">
							                        <input type="file" name="gallery_2_replace[<?= $img ?>]" class="form-control form-control-sm mb-1 replace-input" data-preview-target="<?= $img ?>">
							    ...
							    <button type="button" class="btn btn-primary btn-sm btn-success mt-3" id="add-more-item">Add Team Member</button>
							</div>
							...
							document.getElementById('add-more-item').addEventListener('click', function () {
							    const container = document.getElementById('gallery-2-container');

							    const box = document.createElement('div');
							    box.classList.add('col-md-6', 'gallery-img-box', 'mb-3');

							    // Placeholder image (default profile)
							    const placeholderImg = '<?php echo base_url ?>'+'dist/img/no-image-available.png';
								

							    box.innerHTML = `
							        <img src="${placeholderImg}" class="img-thumbnail mb-1 preview-image" style="height:130px;">
							        <input type="file" name="gallery_2_imgs[]" class="form-control form-control-sm mb-1 image-upload">
							    ...
							    // Delete button for dynamic block
							    box.querySelector('.remove-gallery-img').addEventListener('click', function () {
							        box.remove();
							    });

							    // Image preview
							    box.querySelector('.image-upload').addEventListener('change', function (event) {
							        const file = event.target.files[0];
							        const previewImg = box.querySelector('.preview-image');
							        if (file) {
							    ...
							    container.appendChild(box);
									$(box).find('.singleteamdetaileditor').summernote({
										height: 200,
										toolbar: [
													[ 'style', [ 'style' ] ],
