...
Class Content extends DBConnection {
	...
	public function about_us(){
		...
		/*------------------ Start team gallary 2-----------------------*/
			...
			// Update titles, descriptions, and replace images
			if (isset($_POST['gallery_2_titles_existing'])) {
				foreach ($existing_gallery_2 as $i => $entry) {
					...
					if (!empty($_FILES['gallery_2_replace']['name'][$img])) {
						$new_name = uniqid('img_', true) . "_" . basename($_FILES['gallery_2_replace']['name'][$img]);

						if (move_uploaded_file($_FILES['gallery_2_replace']['tmp_name'][$img], $gallery_dir . $new_name)) {
							// Delete old file
							$old_path = $gallery_dir . $img;
							if (is_file($old_path)) {
								unlink($old_path);
							}
							$img = $new_name; // Replace with new filename
						}
					}

					// ✅ Rebuild entry with updated (or replaced) image
					$existing_gallery_2[$i] = $img
						. (!empty($title) ? "::" . $title : '')
						. (!empty($designation) ? "::" . $designation : '')
